...
def compute_weighted_child_entropy(data, attribute):
    ...
    print(f"Values are \n {values}")
    print(f"Counts are \n {counts}")
    class_lab_attr = [row["Class"] for row in data]
    print(f"Class labels associated with the attribute are: {class_lab_attr}")
    ...
    for unique_value in counts.keys():
        ...
        # print(f"{probability} is the probability for unique value {unique_value}")
        ...
        # print(f"The final entropy value for {attribute} is {final_entropy_val}")
    print(f"Final entropy compute is for attribute {attribute} is: {sum(final_entropy_val)}")
